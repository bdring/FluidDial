...
#ifndef __Touch_Class_H__
#define __Touch_Class_H__
...
namespace m5 {
    ...
    public:
        ...
        struct touch_detail_t : public lgfx::touch_point_t {
            ...
            inline bool wasReleased(void) const {
                return (state & (touch_state_t::mask_touch | touch_state_t::mask_change)) == touch_state_t::mask_change;
            };
            inline bool isHolding(void) const {
                return (state & (touch_state_t::mask_touch | touch_state_t::mask_holding)) ==
                       (touch_state_t::mask_touch | touch_state_t::mask_holding);
            }
        ...
        ///
        inline const touch_detail_t& getDetail(std::size_t index = 0) const {
            return _touch_detail[_touch_raw[index].id < TOUCH_MAX_POINTS ? _touch_raw[index].id : 0];
        }

        inline const lgfx::touch_point_t& getTouchPointRaw(std::size_t index = 0) const {
            return _touch_raw[index < _detail_count ? index : 0];
        }
